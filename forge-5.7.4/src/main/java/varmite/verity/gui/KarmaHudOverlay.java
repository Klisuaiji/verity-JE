...
public class KarmaHudOverlay {
    ...
    public static final IGuiOverlay HUD_KARMA = (gui, guiGraphics, partialTick, width, height) -> {
        ...
        if (((Boolean)VerityConfig.SHOW_VERITYS_KARMA.get()).booleanValue() && !((Boolean)VerityConfig.IMMERSIVE_MODE.get()).booleanValue()) {
            ...
            if (karma < 7) {
                guiGraphics.m_280163_(FACE_ANGRY, faceX, faceY, 0.0f, 0.0f, faceSize, faceSize, faceSize, faceSize);
            } else if (karma < 14) {
                guiGraphics.m_280163_(FACE_NEUTRAL, faceX, faceY, 0.0f, 0.0f, faceSize, faceSize, faceSize, faceSize);
            } else {
                guiGraphics.m_280163_(FACE_HAPPY, faceX, faceY, 0.0f, 0.0f, faceSize, faceSize, faceSize, faceSize);
            }

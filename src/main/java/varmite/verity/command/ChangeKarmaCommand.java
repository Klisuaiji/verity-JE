...
public class ChangeKarmaCommand {
    public static void register(CommandDispatcher<CommandSourceStack> dispatcher) {
        dispatcher.register(Commands.literal("changekarma").requires(source -> source.hasPermission(2)).then(Commands.argument("karma", IntegerArgumentType.integer(0, 20)).executes(context -> {
            ...
            serverLevel.getServer().execute(() -> ModEvents.setAndSyncKarma((ServerLevel)serverLevel, (float)IntegerArgumentType.getInteger((CommandContext)context, "karma")));
            ((CommandSourceStack)context.getSource()).sendSuccess(() -> Component.literal(("Karma updated to " + IntegerArgumentType.getInteger((CommandContext)context, "karma") + ".")), false);
            return 1;
        })));
    }
}

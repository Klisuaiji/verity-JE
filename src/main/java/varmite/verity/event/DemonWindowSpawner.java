...
public class DemonWindowSpawner {
    ...
    public static void onPlayerTick(PlayerTickEvent.Post event) {
        ...
        boolean bl = isIndoors = !level.canSeeSkyFromBelowWater(player.blockPosition());
        if (isIndoors) {
            ...
            for (int x = -5; x <= 5; ++x) {
                for (int y = -1; y <= 2; ++y) {
                    for (int z = -5; z <= 5; ++z) {
                        ...
                        if (level.getBlockState(spawnPos).blocksMotion() || !level.getBlockState(spawnPos.below()).blocksMotion() || (demon = (VerityDemonEntity)ModEntities.VERITY_DEMON_ENTITY.get().create(level)) == null) continue;
                        demon.moveTo((double)spawnPos.getX() + 0.5, spawnPos.getY(), (double)spawnPos.getZ() + 0.5, 0.0f, 0.0f);
                        demon.lookAt((Entity)player, 180.0f, 180.0f);
            ...
            BlockPos safeSpawnPos = targetPos = BlockPos.containing((double)(player.getX() - lookVec.x * 24.0), (double)player.getY(), (double)(player.getZ() - lookVec.z * 24.0));
            ...
            if ((demon = (VerityDemonEntity)ModEntities.VERITY_DEMON_ENTITY.get().create(level)) != null) {
                demon.moveTo((double)safeSpawnPos.getX() + 0.5, safeSpawnPos.getY(), (double)safeSpawnPos.getZ() + 0.5, 0.0f, 0.0f);
                demon.setDemonState(0);
